...
if [ "$DB_AVAILABLE" = false ]; then
  ...
  PORT=${DB_PORT} NODE_ENV=${NODE_ENV} docker-compose -p service-sql-data-${NODE_ENV} up -d service-sql-db

  COUNT=0

  until docker exec -t service-sql-db-${NODE_ENV} psql -h "localhost" -U ${DB_CORE__USER} -c '\q'; do
