...
test('Not restricted, no auth', async () => {
  ...
  await broker.call('service-sql.ping', undefined, {});
...
test('Restricted, no auth', async () => {
  await expect(
    (async () => broker.call('service-sql.pingAuth', undefined, {}))()
...
test('Restricted, invalid auth', async () => {
  await expect(
    (async () =>
      broker.call('service-sql.pingAuth', undefined, {
...
test('Restricted, with auth', async () => {
  const pingSpy = jest.spyOn(ping, 'ping');
  await broker.call('service-sql.pingAuth', undefined, {
    meta: { auth: sudoAuth }
  });
