...
let dbConnector: MikroConnector<MongoDriver> | null = null;
let initError: Error | null = null;

let pending: null | EventEmitter = null;
let closing = false;

export async function getDbConnector(): Promise<MikroConnector<MongoDriver>> {
  if (closing) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const tmpConnector = new MikroConnector<MongoDriver>();
