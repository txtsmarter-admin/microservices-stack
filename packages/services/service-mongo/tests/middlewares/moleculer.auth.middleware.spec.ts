...
test('Not restricted, no auth', async () => {
  ...
  await broker.call('service-mongo.ping', undefined, {});
...
test('Restricted, no auth', async () => {
  await expect(
    (async () => broker.call('service-mongo.pingAuth', undefined, {}))()
...
test('Restricted, invalid auth', async () => {
  await expect(
    (async () =>
      broker.call('service-mongo.pingAuth', undefined, {
...
test('Restricted, with auth', async () => {
  const pingSpy = jest.spyOn(ping, 'ping');
  await broker.call('service-mongo.pingAuth', undefined, {
    meta: { auth: sudoAuth }
  });
